    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 translate-y-3 p-4 text-white transition-transform duration-300 group-hover:translate-y-0">
        <div className="space-y-2">
          <div className="flex items-start justify-between gap-2">
            <h3 className="line-clamp-2 text-lg font-semibold leading-tight drop-shadow-[0_6px_18px_rgba(0,0,0,0.45)]">
              {title}
            </h3>
            <Badge
              variant="secondary"
              className="shrink-0 border border-yellow-200/50 bg-yellow-400/95 text-black shadow-[0_0_18px_rgba(250,204,21,0.35)]"
            >
          ...
          <div className="opacity-0 transition-opacity duration-300 group-hover:opacity-100">
            ...
            >
              {actionLabel ?? (isComingSoon ? 'View details' : 'Book now')}
            </Button>
          </div>
        </div>
      </div>
      ...
      {isComingSoon && (
        <div className="absolute right-2 top-2 [&_span]:border [&_span]:border-cyan-200/35 [&_span]:shadow-[0_0_20px_rgba(34,211,238,0.28)]">
          <Badge className="bg-gradient-to-r from-blue-600 to-cyan-600">Coming soon</Badge>
        </div>
      )}
    </div>
  );
